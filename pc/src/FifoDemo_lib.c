...
SCILIB int REG_events_GET(uint32_t *val, NI_HANDLE *handle)
{
     return __abstracted_reg_read(val, SCI_REG_events, handle);
}
SCILIB int REG_events_SET(uint32_t val, NI_HANDLE *handle)
{
     return __abstracted_reg_write(val, SCI_REG_events, handle);
}
//-----------------------------------------------------------------
//-
//- CPACK_CP_0_START
...
SCILIB int CPACK_CP_0_RECONSTRUCT_DATA(void *buffer_handle, t_FRAME_packet_collection *decoded_packets)
{
	...
	//allocate output data packets. Extimate extra space for extra
	//packets
